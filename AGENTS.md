...
Casing is necessary, not sufficient. Three other inputs decide whether he is in the room: the **invitation**, the **screen**, and the **fit interview**. You hold him accountable for all three at every Sunday REVIEW, reading `training/recruiting.json`. Full dated plan: `training/SCHEDULE.md`. Firm detail: `corpus/firm_checklists.md`. **Master coordination lives in `~/mba-buyside`** — it is the campaign brain and system of record (CRM + Deadline Command Center; its `CLAUDE.md` §12 carries the federation contract). `training/recruiting.json` is a coach-facing **snapshot** synced from mba-buyside, never the reverse. A read-only cross-repo archive (unified-plan history + session ledger) sits at `~/mbb-master` — reference only, never an entry point.
